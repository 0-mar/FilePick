...
def test_filepick(source: str, correct_files: Set[str]) -> None:
    ...
    assert set(os.listdir(TARGET_DIR)) == correct_files, \
        f"Neshodují se zkopírované soubory - zkopírovalo se {set(os.listdir(TARGET_DIR))}, správně " \
...
def test():
    ...
    test_filepick(os.path.join("test", "1"), {'a.txt', 'c.docx'})
    ...
    sys.argv[3] = ".wtf"
    sys.argv[4] = ".png"
    test_filepick(os.path.join("test"), set())

    sys.argv.append("filepick.py")
    sys.argv[5] = "-r"
    test_filepick(os.path.join("test"), {"e.wtf", "b.png"})
